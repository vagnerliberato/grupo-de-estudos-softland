...
public class DiarioFrame extends JInternalFrame {
        ...
        try {
            ...
            if (diarioDao.isDiaAberto(Funcoes.getDataAtual())) {
                ...
                if (!diarioDao.isValidaAberturaFechamento(Funcoes.getDataAtual())) {
                    Funcoes.alerta(this, "O diario de hoje já foi fechado!");
                } else {
                    Funcoes.alerta(this, "Já existe um diário aberto para o dia "
                            + Funcoes.getDataString(Funcoes.getDataAtual()));
                }
            } else if (JOptionPane.showConfirmDialog(this,
                    "Ainda não foi criado um diario para o dia "
                    + Funcoes.getDataString(Funcoes.getDataAtual()) + ". \n\n Deseja criar ?",
                    "Novo Diário",
                ...
                if (result > 0) {
                    Funcoes.alerta(this, "Beleza, seu diário foi criado com sucesso!");
                } else {
        ...
        try {
            if ((diarioDao.isDiaAberto(dData)) && (diarioDao.isValidaAberturaFechamento(dData))) {
                if (JOptionPane.showConfirmDialog(this,
                        "Confirma o fechamento de seu diário para o dia "
                        + Funcoes.getDataString(dData),
                        "Fechamento",
                        JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
                    ...
                    if (result > 0) {
                        Funcoes.alerta(this, "Diário fechado com sucesso, até amanhã!");
                    } else {
                        Funcoes.alerta(this, "Ocorreu algum problema, verifique...");
                    }
                }
            } else {
                Funcoes.alerta(this, "Não existe um diário aberto para o dia " + Funcoes.getDataString(dData));
            }
        } catch (Exception ex) {
            Funcoes.alerta(this, ex.getMessage());
        }
